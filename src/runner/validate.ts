...
export async function validateFile(filePath: string, rules?: ValidateRule[]): Promise<ValidateResult> {
  const rs = rules ?? DEFAULT_VALIDATE_RULES;
  for (const rule of rs) {
    const r = await checkOne(filePath, rule);
...
export async function validateFiles(outputSpec: string, cwd: string, rules?: ValidateRule[]): Promise<ValidateResult> {
...
async function checkOne(filePath: string, rule: ValidateRule): Promise<ValidateResult> {
...
// 运行时语法检查（异步 exec，超时 10s；不阻塞事件循环，MCP server 可并发处理其他调用）
function runCheck(filePath: string, checker: string): Promise<ValidateResult> {
  ...
  return new Promise((resolve) => {
    exec(cmd, { timeout: 10000 }, (err, _stdout, stderr) => {
      ...
      const tail = (stderr ?? "").toString().slice(0, 200) || err.message;
      resolve({ passed: false, failedRule: rule, detail: `${checker} failed: ${tail}` });
    });
  });
}
